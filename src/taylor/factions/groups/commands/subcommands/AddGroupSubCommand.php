...
class AddGroupSubCommand extends BaseSubCommand {

    public function __construct() {
        parent::__construct(Main::getInstance(), "add", "Add a group to a player.");
    }
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args) : void {
        ...
        if ($playerSession->hasGroup($group)) {
            $sender->sendMessage($session->getMessage("commands.group.hasGroup"));
            return;
        }
        $playerSession->addGroup($group);
        $sender->sendMessage($session->getMessage("commands.group.groupAdded", [["{group}", "{player}"], [$group, $player->getName()]]));
    }
